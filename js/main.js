...
async function getWeather(country) {
  var apiResponse = await fetch(
    `https://api.weatherapi.com/v1/forecast.json?key=469863d1f6cd40f8bd3223044241401&q=${country}&days=3`
  );
...
function displayData(apiData) {
  var wetherInfo = apiData.forecast.forecastday;
  var cityInfo = apiData.location.name;
  var currentDay = apiData.current;
  ...
  for (var i = 0; i < wetherInfo.length; i++) {
    ...
    cartona += `<div id = "test" class="box col-lg-3 p-0 m-0 mb-3 rounded-4 text-center">
        ...
        <div class="mb-0"><img src="https:${
          wetherInfo[i].day.condition.icon
        }" class="w-25"></div>
        <div class="fs-1 mt-1 fw-bolder"><p>${
          wetherInfo[i].day.avgtemp_c
        }°C<p></div>
        <div><p class="fw-bolder fs-4 ">${
          wetherInfo[i].day.condition.text
        }</p></div>
        <div class="text-white pb-2">
            <span><img src="images/icon-umberella.png"> ${
              wetherInfo[i].day.daily_chance_of_rain
            }%</span>
            <span class="px-2"><img src="images/icon-wind.png"> ${
              wetherInfo[i].day.maxwind_kph
            }km/h</span>
            <span><img src="images/icon-compass.png"> ${
              wetherInfo[i].hour[0].wind_dir
            }</span>
        </div>
  ...
  document.getElementById("maincard").innerHTML = cartona;
  ...
  var adjust1stCard = ` 
    ...
    <div class="fs-1 mt-1 text-white fw-bolder"><p>${
      currentDay.temp_c
    }°C<p></div>
    <div><p class="fw-bolder fs-4 text-white">${
      currentDay.condition.text
    }</p></div>
    <div class="text-white fw-bold pb-2">
        <span><img src="images/icon-umberella.png"> ${
          wetherInfo[0].day.daily_chance_of_rain
        }%</span>
        <span class="px-2"><img src="images/icon-wind.png"> ${
          currentDay.wind_kph
        }km/h</span>
  ...
  document.getElementById("test").innerHTML = adjust1stCard;
}
